...
public class ControladorPizzaBase {
    ...
    public boolean ingresarPizza (String nombre, String tamano, String presentacion, int precio,byte [] imagen) throws Exception
    {
        boolean operacion = false;
        Pizzabase pizza = new Pizzabase( nombre, tamano, presentacion, precio);
        pizza.setFoto(imagen);
        try 
        {
            logica.create(pizza);
            operacion = true;
        ...
        public boolean BorrarPizza (Integer id)
    {
        ...
        return operacion;
          ...
          public Iterator <Pizzabase>  getPizzas ()
        {
           Iterator <Pizzabase>  pizzas = logica.findPizzabaseEntities().iterator();
